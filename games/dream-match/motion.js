...
(() => {
  ...
  async function swap(board,i,j){
    ...
    await Promise.all([[A,b.x-a.x,b.y-a.y],[B,a.x-b.x,a.y-b.y]].flatMap(([cell,x,y])=>pieces(cell).map(el=>play(el,[{translate:'0 0'},{translate:x+'px '+y+'px'}],{duration:230,easing:'cubic-bezier(.3,.05,.25,1)',fill:'forwards'}))));
  ...
  function fall(board,distances){
    ...
    return Promise.all(Object.entries(distances).filter(([,d])=>d>0).flatMap(([i,d])=>pieces(board.children[i]).map(el=>play(el,[{translate:'0 '+(-d*step)+'px',opacity:.3,offset:0},{translate:'0 2px',opacity:1,offset:.86},{translate:'0 0',opacity:1,offset:1}],{duration:Math.min(500,230+d*38),delay:(i%7)*12,easing:'cubic-bezier(.32,.05,.55,1)',fill:'backwards'}))));
  }
  function spark(cell){
  ...
  async function clear(board,indices,made,activated,colors,goals){
    ...
    for(const i of indices){
      ...
      work.push(...pieces(cell).map(el=>play(el,[{scale:1,opacity:1},{scale:1.2,opacity:1,offset:.3},{scale:.15,opacity:0}],{duration:330,fill:'forwards',easing:'ease-in'})),spark(cell));
    ...
    for(const i of Object.keys(made))work.push(spark(board.children[i]),...pieces(board.children[i]).map(el=>play(el,[{scale:.4},{scale:1.22,offset:.65},{scale:1}],{duration:420,easing:'ease-out'})));
  ...
  function shuffle(board,out){for(const animation of active)animation.cancel();return Promise.all([...board.children].flatMap((cell,i)=>pieces(cell).map(el=>play(el,out?[{scale:1,opacity:1,rotate:'0deg'},{scale:.2,opacity:0,rotate:'-16deg'}]:[{scale:.2,opacity:0,rotate:'16deg'},{scale:1,opacity:1,rotate:'0deg'}],{duration:240,delay:(i%7)*13,easing:'ease-in-out',fill:out?'forwards':'backwards'}))));}
  function feedback(el){return play(el,[{scale:.92,opacity:0},{scale:1.04,opacity:1,offset:.6},{scale:1,opacity:1}],{duration:240});}
  function hint(board,indices){if(reduced())return;for(const i of indices)for(const el of pieces(board.children[i]))play(el,[{scale:1},{scale:1.12},{scale:1}],{duration:850,easing:'ease-in-out'});}
