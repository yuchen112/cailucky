...
vm.runInContext(
  ...
  if(new Set(EVENTS.slice(0,12).map(e=>e[3])).size!==12||EVENTS.slice(0,12).some(e=>!IM['event_'+e[3]]?.complete))throw new Error('dedicated event art is not wired for the completed event set');
