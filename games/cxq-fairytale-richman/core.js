...
load('homeBg',A+'backgrounds/home_scene_v6.png');load('setupBg',A+'backgrounds/setup_scene_v3.webp');load('mapBg',A+'backgrounds/map_scene_r1.webp');
...
function home(){
  ...
  homeButton('start','新遊戲',88,356,474,104,true,0);homeButton('continue','繼續遊戲',112,474,426,76,false,1,canContinue);homeButton('help','遊戲說明',112,564,426,76,false,2);homeButton('settings','系統設定',112,654,426,76,false,3);
  const infoA=homeIntro(740,380);X.save();X.globalAlpha=infoA;contain(IM.roleInfo,104,758,445,92,.93);txt(canContinue?'已有冒險紀錄，可從上次進度繼續':'尚無冒險紀錄｜開始新遊戲建立存檔',327,797,15,'center','#60462d',900,false);txt('2–4 人｜真人 / AI 自由配置',327,824,13,'center','#76583b',800,false);X.restore();
...
const SETUP_VIEW={char:S.seats[S.activeSeat].char};
...
function seatPanel(i,x,y){const s=S.seats[i],sel=S.activeSeat===i;contain(IM.playerSeat,x,y,368,148,s.type==='off'?.40:(sel?1:.94));S.buttons.push({id:'seat'+i,x,y,w:368,h:148,en:!S.pickAnim});if(!(S.pickAnim&&S.pickAnim.seat===i)&&s.type!=='off')contain(IM['c'+s.char],x+14,y+10,104,112,.99);txt((i+1)+'P',x+151,y+31,17,'center',sel?'#ffe67a':'#fff',1000,true);txt(s.type==='off'?'空席':CHAR_NAMES[s.char],x+247,y+31,15,'center','#fff',900,true);btn('human'+i,'真人',x+126,y+70,68,36,false,s.type==='human'?1:.38,!S.pickAnim);btn('ai'+i,'AI',x+199,y+70,56,36,true,s.type==='ai'?1:.38,!S.pickAnim);btn('off'+i,'空席',x+260,y+70,72,36,false,s.type==='off'?1:.38,!S.pickAnim);if(s.type==='ai')btn('diff'+i,s.diff==='easy'?'輕鬆':s.diff==='standard'?'標準':'聰明',x+252,y+108,86,30,false,.9,!S.pickAnim)}
function setup(){updatePickAnim();cover(IM.setupBg,0,0,W,H,1);btn('back','返回首頁',18,15,170,58,false,1,!S.pickAnim);txt('角色選擇',800,44,38,'center','#fff0b6',1000,true);txt('先選擇玩家席位，再挑選角色',800,82,16,'center','#fff',800,true);const ci=SETUP_VIEW.char,owner=assigned(ci);contain(IM.charSlot,SETUP.stageX,SETUP.stageY,SETUP.stageW,SETUP.stageH,.98);if(!(S.pickAnim&&S.pickAnim.char===ci))contain(IM['c'+ci],SETUP.stageX+72,SETUP.stageY+38,SETUP.stageW-144,SETUP.stageH-96,.99);btn('prevChar','◀',168,310,112,94,false,1,!S.pickAnim);btn('nextChar','▶',790,310,112,94,false,1,!S.pickAnim);txt(`${ci+1} / ${CHAR_KEYS.length}`,535,594,16,'center','#fff5cf',900,true);btn('confirmChar',owner===undefined||owner===S.activeSeat?'選擇這名角色':`與 ${owner+1}P 交換`,385,610,300,62,true,1,!S.pickAnim&&S.seats[S.activeSeat].type!=='off');contain(IM.roleInfo,945,116,575,220,.98);txt(CHAR_NAMES[ci],1220,155,30,'center','#50321d',1000,false);txt(CHAR_ROLES[ci],1220,202,19,'center','#7a4d2d',1000,false);txt(ROLE_DESC[ci],1220,240,16,'center','#745236',850,false);txt(`目前設定：${S.activeSeat+1}P`,1220,284,18,'center',PLAYER_COLORS[S.activeSeat],1000,true);contain(IM['c'+ci],1018,315,210,285,.94);btn('money','起始資金 $'+S.money.toLocaleString(),1240,350,300,58,false,.96,!S.pickAnim);btn('rounds',S.rounds+' 回合',1240,420,300,58,false,.96,!S.pickAnim);txt(`參賽 ${activeSeatIds().length} 人｜真人 ${humanCount()} 人`,1390,500,15,'center','#fff',850,true);btn('startGame','下一步',1240,535,300,76,true,1,activeSeatIds().length>=2&&humanCount()>=1&&!S.pickAnim);for(let i=0;i<4;i++)seatPanel(i,SETUP.seatX[i],SETUP.seatY);drawPickAnim()}
