...
function result(){const b=S.board;cover(IM.setupBg,0,0,W,H,.82);txt('冒險結算',800,58,46,'center','#fff0a5',1000,true);txt(`${b.mapRules?.name||'童話王國'}｜${b.victory==='survival'?'最後生存者':'總資產競賽'}｜${Math.max(1,b.round-1)} 回合`,800,105,18,'center','#fff',850,true);const ranked=b.players.slice().sort((a,z)=>a.bankrupt!==z.bankrupt?(a.bankrupt?1:-1):netWorth(z)-netWorth(a));ranked.forEach((p,i)=>{const y=150+i*142;contain(IM['playerSeatP'+((p.id%4)+1)],230,y,1140,126,p.bankrupt?.58:.98);contain(IM['portrait'+p.char],260,y+9,110,108,p.bankrupt?.48:1);txt(`${i+1}`,403,y+63,38,'center',i===0?'#ffe071':'#fff',1000,true);txt(`${p.id+1}P　${CHAR_NAMES[p.char]}`,455,y+37,24,'left','#fff',1000,true);txt(p.bankrupt?'已破產':'完成冒險',455,y+78,16,'left',p.bankrupt?'#ffb7b7':'#dfffdc',850,true);const lands=b.tiles.filter(t=>t.owner===p.id).length;txt(`現金　$${p.cash.toLocaleString()}`,790,y+38,18,'left','#fff6d2',900,true);txt(`土地　${lands} 塊`,790,y+77,17,'left','#dbeaff',850,true);txt(`總資產　$${netWorth(p).toLocaleString()}`,1115,y+58,20,'center',i===0?'#ffe071':'#fff',1000,true)});btn('resultHome','返回首頁',420,765,330,78,false);btn('rematch','同設定再戰',850,765,330,78,true)}
