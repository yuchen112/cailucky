...
const MAPS=[
  {key:'starwish',name:'星願花園',tag:'初次冒險',difficulty:'輕鬆',desc:['道路平穩、價格均衡','適合第一次遊玩'],regions:['星願花圃','月輝城鎮','森語溪谷','玫瑰商街'],priceRate:1,rentRate:1,eventRate:1},
  {key:'moonharbor',name:'月光港灣',tag:'潮汐之路',difficulty:'標準',desc:['租金較高、卡片活躍','港口事件改變局勢'],regions:['月港碼頭','潮汐街區','燈塔山坡','星砂商埠'],priceRate:1.08,rentRate:1.15,eventRate:1.2},
  {key:'cloudbazaar',name:'雲端市集',tag:'空島競逐',difficulty:'挑戰',desc:['土地昂貴、事件頻繁','適合熟悉規則的玩家'],regions:['風鈴雲階','彩霞工坊','飛艇驛站','天穹商街'],priceRate:1.18,rentRate:1.28,eventRate:1.45}
];
...
load('playerSeat',A+'ui/player_seat_v2.webp');load('roleInfo',A+'ui/role_info_v2.webp');
...
for(let i=0;i<4;i++)load('playerSeatP'+i,A+`ui/player_seat_wide_p${i+1}_v1.webp`);
...
MAPS.forEach((m,mi)=>{
  for(let level=1;level<=3;level++)load(`building${mi}_${level}`,A+`buildings/${m.key}_l${level}_v1.webp`);
...
function sceneBackdrop(){if(S.scene==='home')return IM.homeBg;if(['setup','mapSelect','rules','result','help','settings'].includes(S.scene))return IM.setupBg;if(S.scene==='game'&&S.board)return IM['mapWorld'+(S.board.mapIndex||0)];return IM.homeBg}
function begin(){X.setTransform(1,0,0,1,0,0);X.fillStyle='#071325';X.fillRect(0,0,C.width,C.height);const edgeBg=sceneBackdrop();if(edgeBg&&edgeBg.complete&&edgeBg.naturalWidth){const r=Math.max(C.width/edgeBg.naturalWidth,C.height/edgeBg.naturalHeight),iw=edgeBg.naturalWidth*r,ih=edgeBg.naturalHeight*r;X.save();X.globalAlpha=.78;X.filter='blur(14px) saturate(.86) brightness(.62)';X.drawImage(edgeBg,(C.width-iw)/2,(C.height-ih)/2,iw,ih);X.filter='none';X.restore()}X.setTransform(VIEW.scale,0,0,VIEW.scale,VIEW.ox,VIEW.oy);S.buttons=[]}
...
function fitTxt(s,x,y,maxW,z=26,a='center',c='#fff',w=800,o=true,min=12){let size=z;X.save();while(size>min){X.font=`${w} ${size}px system-ui,-apple-system,"Noto Sans TC",sans-serif`;if(X.measureText(String(s)).width<=maxW)break;size--}X.restore();txt(s,x,y,size,a,c,w,o);return size}
function wrapLines(s,maxW,z=22,w=800,maxLines=3){const chars=Array.from(String(s)),lines=[];let line='';X.save();X.font=`${w} ${z}px system-ui,-apple-system,"Noto Sans TC",sans-serif`;for(const ch of chars){const next=line+ch;if(line&&X.measureText(next).width>maxW){lines.push(line);line=ch;if(lines.length===maxLines-1)break}else line=next}const used=lines.join('').length;if(lines.length===maxLines-1&&used+line.length<chars.length){while(line.length&&X.measureText(line+'…').width>maxW)line=line.slice(0,-1);line+='…'}if(line)lines.push(line);X.restore();return lines.slice(0,maxLines)}
function paragraph(s,x,y,maxW,z=22,lineH=30,maxLines=3,a='center',c='#fff',w=800,o=true){const lines=wrapLines(s,maxW,z,w,maxLines),start=y-(lines.length-1)*lineH/2;lines.forEach((line,i)=>txt(line,x,start+i*lineH,z,a,c,w,o));return lines.length}
...
function btn(id,label,x,y,w,h,red=false,alpha=1,en=true){contain(red?IM.btnRed:IM.btnBlue,x,y,w,h,alpha*(en?1:.38));fitTxt(label,x+w/2,y+h*.49,w*.72,Math.min(29,h*.34),'center','#fff',900,true,12);S.buttons.push({id,x,y,w,h,en})}
...
const SETUP={stageX:260,stageY:104,stageW:520,stageH:520,seatY:728,seatX:[5,405,805,1205]};
const SETUP_VIEW={char:S.seats[S.activeSeat].char,notice:'',noticeUntil:0};
function seatTarget(i){return{x:SETUP.seatX[i]+78,y:SETUP.seatY+72}}
...
function seatPanel(i,x,y){
  ...
  const alpha=s.type==='off'?.48:(sel?1:.94);stretch(IM['playerSeatP'+i],x,y,390,150,alpha);
  S.buttons.push({id:'seat'+i,x,y,w:390,h:150,en:!S.pickAnim});
  if(!(S.pickAnim&&S.pickAnim.seat===i)&&s.type!=='off')contain(IM['portrait'+s.char],x+24,y+20,108,108,.99);
  contain(s.type==='human'?IM.statusHuman:s.type==='ai'?IM.statusAi:IM.statusOff,x+105,y+102,42,42,s.type==='off'?.72:1);
  txt((i+1)+'P',x+170,y+30,19,'center',sel?'#ffe67a':'#fff',1000,true);fitTxt(s.type==='off'?'尚未參賽':CHAR_NAMES[s.char],x+290,y+30,170,16,'center',i===3?'#fff3cf':'#fff',950,true,12);
  btn('seatType'+i,typeLabel,x+154,y+50,210,45,s.type==='human',s.type==='off'?.55:1,!S.pickAnim);
  if(s.type==='ai')btn('diff'+i,'AI 難度：'+(s.diff==='easy'?'輕鬆':s.diff==='standard'?'標準':'聰明'),x+154,y+99,210,39,false,.94,!S.pickAnim);else fitTxt(s.type==='human'?'點擊切換為電腦':'點擊重新加入',x+259,y+119,194,13,'center','#fff5cf',850,true,10)
}
function setup(){updatePickAnim();cover(IM.setupBg,0,0,W,H,1);btn('back','返回首頁',18,15,190,64,false,1,!S.pickAnim);txt('角色選擇',800,42,40,'center','#fff0b6',1000,true);txt('先選席位，再選角色｜真人・電腦・空席自由配置',800,80,16,'center','#fff',850,true);const ci=SETUP_VIEW.char,owner=assigned(ci);if(!(S.pickAnim&&S.pickAnim.char===ci))contain(IM['c'+ci],SETUP.stageX+105,SETUP.stageY+72,SETUP.stageW-210,SETUP.stageH-120,.99);contain(IM.characterStage,SETUP.stageX,SETUP.stageY,SETUP.stageW,SETUP.stageH,.99);btn('prevChar','◀',132,306,108,96,false,1,!S.pickAnim);btn('nextChar','▶',672,306,108,96,false,1,!S.pickAnim);txt(`${ci+1} / ${CHAR_KEYS.length}`,520,588,19,'center','#fff5cf',900,true);btn('confirmChar',owner===undefined||owner===S.activeSeat?'選擇這名角色':`與 ${owner+1}P 交換`,350,606,340,68,true,1,!S.pickAnim&&S.seats[S.activeSeat].type!=='off');contain(IM.abilityPanel,850,104,420,420,.99);fitTxt(CHAR_NAMES[ci],1060,164,310,30,'center','#fff4c8',1000,true,20);fitTxt(CHAR_TITLES[ci]+'｜'+CHAR_ROLES[ci],1060,207,300,17,'center','#d9eaff',950,true,13);paragraph(CHAR_CONCEPTS[ci],1060,284,292,16,25,2,'center','#fff',850,true);txt('專屬能力',1060,348,15,'center','#ffe17b',1000,true);paragraph(ROLE_DESC[ci],1085,407,260,15,24,2,'center','#fff6d6',900,true);txt('★',935,407,32,'center','#75d8ff',1000,true);txt(`目前設定：${S.activeSeat+1}P`,1060,473,18,'center',PLAYER_COLORS[S.activeSeat],1000,true);btn('money','起始資金 $'+S.money.toLocaleString(),1278,174,300,64,false,.96,!S.pickAnim);btn('rounds',S.rounds+' 回合',1278,250,300,64,false,.96,!S.pickAnim);txt(`參賽 ${activeSeatIds().length} 人｜真人 ${humanCount()} 人`,1428,347,17,'center','#fff',900,true);btn('startGame','下一步',1268,388,310,86,true,1,activeSeatIds().length>=2&&humanCount()>=1&&!S.pickAnim);if(SETUP_VIEW.notice&&performance.now()<SETUP_VIEW.noticeUntil)fitTxt(SETUP_VIEW.notice,1060,558,380,17,'center','#ffe27a',1000,true,13);for(let i=0;i<4;i++)seatPanel(i,SETUP.seatX[i],SETUP.seatY);drawPickAnim()}

function mapSelect(){
  cover(IM.setupBg,0,0,W,H,.82);txt('選擇冒險地圖',800,52,42,'center','#fff0b6',1000,true);txt('縮圖、路線與實際棋盤主題完全對應',800,94,17,'center','#fff',850,true);btn('mapBack','返回選角',22,20,190,62,false);
  const xs=[55,575,1095];
  MAPS.forEach((m,i)=>{const x=xs[i],selected=S.mapIndex===i;X.save();X.beginPath();X.roundRect(x+50,182,350,222,12);X.clip();cover(IM['mapPreview'+i],x+50,182,350,222,selected?1:.67);X.restore();stretch(IM.mapCardFrame,x,126,450,600,selected?1:.8);if(selected){X.save();X.strokeStyle='#ffe27a';X.shadowColor='#ffd65b';X.shadowBlur=20;X.lineWidth=6;X.beginPath();X.roundRect(x+9,135,432,574,28);X.stroke();X.restore()}fitTxt(m.name,x+225,425,330,29,'center','#fff2bd',1000,true,20);fitTxt(m.tag+'｜'+m.difficulty,x+225,463,330,17,'center','#d9eaff',900,true,13);paragraph(m.desc.join('，'),x+225,512,330,16,25,2,'center','#fff',850,true);txt(`土地 ×${m.priceRate.toFixed(2)}　租金 ×${m.rentRate.toFixed(2)}`,x+225,568,14,'center','#ffe17b',850,true);txt(`事件 ×${m.eventRate.toFixed(2)}`,x+225,596,14,'center','#d8f2ff',850,true);btn('chooseMap'+i,selected?'已選擇':'選擇地圖',x+75,634,300,62,selected,false,1)});
...
function buildingImage(t){if(t.owner<0||t.level<1)return null;const mi=S.board?.mapIndex||0,isLandmark=t.level>=3&&regionOwned(t.owner,t.region);return IM[`building${mi}_${isLandmark?'landmark':Math.min(3,t.level)}`]}
function drawTile(t){contain(tileImage(t.type),t.x-88,t.y-88,176,176,1);if(t.type==='start'||t.type==='event')txt(t.type==='start'?'起點':'事件',t.x,t.y+4,16,'center','#fff6d2',1000,true);if(t.type==='land'){if(t.owner>=0){const pulse=S.board?.buildAnim?.tile===t?Math.max(0,1-(performance.now()-S.board.buildAnim.at)/850):0;contain(IM.btnRed,t.x-42,t.y-111,84,39,.92);txt((t.owner+1)+'P',t.x,t.y-92,14,'center',PLAYER_COLORS[t.owner],1000,true);const building=buildingImage(t);if(building){const grow=1+pulse*.18,sz=152*grow;X.save();if(pulse){X.shadowColor='#ffe374';X.shadowBlur=28*pulse}contain(building,t.x-sz/2,t.y-177-(sz-152),sz,sz,1);X.restore();if(pulse>0)txt('★ 升級完成 ★',t.x,t.y-190,14,'center','#ffe274',1000,true)}else if(pulse>0)txt('★ 地契已取得 ★',t.x,t.y-137,14,'center','#ffe274',1000,true)}txt('$'+Math.round(t.price/1000)+'K',t.x,t.y+69,13,'center','#fff6d2',900,true)}}
...
function drawMap(){const b=S.board;stretch(IM['mapWorld'+(b.mapIndex||0)]||IM.mapWorld0,0,0,MW,MH,1);b.tiles.forEach(drawTile);if(b.npcs)for(const n of b.npcs){const t=b.tiles[n.pos];if(t)npcMarker(n,t)}drawPlayers()}
function playerHudCard(p,i){const b=S.board,x=12+i*286,y=12,w=274,h=106,current=p.id===cp().id;stretch(IM['playerSeatP'+p.id],x,y,w,h,current?1:.82);contain(IM['portrait'+p.char],x+7,y+10,80,80,p.bankrupt?.45:1);fitTxt(`${p.id+1}P ${CHAR_NAMES[p.char]}`,x+98,y+22,162,18,'left',current?'#ffe894':'#fff',1000,true,13);fitTxt(`現金 $${Math.max(0,p.cash).toLocaleString()}`,x+98,y+47,162,15,'left','#fff5d3',900,true,11);const land=b.tiles.filter(t=>t.owner===p.id).length;fitTxt(`資產 $${netWorth(p).toLocaleString()}`,x+98,y+70,162,14,'left','#d9efff',850,true,10);txt(`土地 ${land}　卡片 ${p.cards.length}`,x+98,y+91,12,'left','#d7e6f4',800,true);const fx=(p.effects||[]).map(e=>`${e.kind}${e.turns}`).join(' ');if(fx)fitTxt(fx,x+w-8,y+91,78,10,'right','#ffe07a',900,true,8);if(current){X.save();X.strokeStyle='#fff4ad';X.lineWidth=4;X.beginPath();X.roundRect(x-2,y-2,w+4,h+4,20);X.stroke();X.restore()}}
function hud(){const b=S.board,p=cp();b.players.forEach(playerHudCard);stretch(IM.roleInfo,1170,12,416,106,.96);fitTxt(`${b.mapRules?.name||'童話王國'}　　第 ${b.round}/${S.rounds} 回合`,1378,38,360,18,'center','#fff4c9',1000,true,12);fitTxt(`現在行動：${p.id+1}P ${CHAR_NAMES[p.char]}`,1378,69,350,17,'center',PLAYER_COLORS[p.id],1000,true,12);fitTxt(p.type==='ai'?`電腦思考中・${p.diff==='easy'?'輕鬆':p.diff==='smart'?'聰明':'標準'}`:'請選擇本回合行動',1378,95,350,13,'center','#e2f1ff',850,true,10);btn('pause','選單',20,130,150,54,false,.94,!S.rolling&&!b.popup);btn('roster','玩家資料',180,130,190,54,false,.94,!S.rolling&&!b.popup);contain(IM['dice'+S.dice],1325,620,145,145);btn('roll','擲骰子',1260,770,300,86,true,1,!S.rolling&&!b.popup&&!b.winner);btn('cards','卡片 '+p.cards.length,1055,786,180,60,false,.95,!S.rolling&&!b.popup);if(S.msg)fitTxt(S.msg,800,850,760,16,'center','#fff6d2',800,true,12)}
...
if(q.kind==='tile'){const t=q.tile;title=t.type==='land'?REGION_NAMES[t.region]+'地產':typeName(t.type);if(t.type==='land'){if(t.owner<0){const baseRent=Math.round(t.price*.25*(b.mapRules?.rentRate||1));body=`空地售價 $${buyCost(p,t).toLocaleString()}｜基礎租金 $${baseRent.toLocaleString()}｜購買後可升級三階建築`;actions=[['buy','購買土地'],['skip','暫時略過']]}else if(t.owner===p.id){const names=['空地','童話小屋','精緻旅店','豪華地標'];body=t.level<3?`${names[t.level]}｜目前租金 $${rentFor(t,p).toLocaleString()}｜升級費 $${Math.round(t.price*.65).toLocaleString()}`:`${regionOwned(p.id,t.region)?'區域地標已落成':'最高級建築'}｜目前租金 $${rentFor(t,p).toLocaleString()}`;actions=t.level<3?[['upgrade',`升級至 ${names[t.level+1]}`],['skip','完成回合']]:[['skip','完成回合']]}else{const owner=b.players.find(x=>x.id===t.owner);body=`${owner?owner.id+1+'P '+CHAR_NAMES[owner.char]:'對手'}的 Lv${t.level} 地產｜需支付 $${rentFor(t,p).toLocaleString()} 租金`;actions=[['pay',p.shield>0?'使用護盾／結算':'支付租金']]} }else if(t.type==='start'){body='在起點稍作休息，獲得旅費。';actions=[['ok','繼續冒險']]}else{body=`抵達${typeName(t.type)}格`;actions=[['special','查看結果']]}}
...
else if(q.kind==='roster'){title='玩家與資產';body='查看目前現金、土地、建築、卡片與總資產';b.players.forEach(x=>{const lands=b.tiles.filter(t=>t.owner===x.id),buildings=lands.reduce((n,t)=>n+t.level,0);actions.push(['inspectPlayer'+x.id,`${x.id+1}P ${CHAR_NAMES[x.char]}｜$${netWorth(x).toLocaleString()}｜地${lands.length} 建${buildings}`])});actions.push(['closeCards','返回棋盤'])}
else if(q.kind==='playerDetail'){const x=q.player,lands=b.tiles.filter(t=>t.owner===x.id),buildings=lands.reduce((n,t)=>n+t.level,0);title=`${x.id+1}P ${CHAR_NAMES[x.char]}`;body=`現金 $${x.cash.toLocaleString()}｜總資產 $${netWorth(x).toLocaleString()}｜土地 ${lands.length}｜建築 ${buildings}｜卡片 ${x.cards.length}`;actions=[['rosterBack','返回玩家列表'],['closeCards','返回棋盤']]}
else if(q.kind==='winner'){title='本局結果';body=q.text;actions=[['home','返回首頁']]}
fitTxt(title,800,205,610,38,'center','#fff0a5',1000,true,22);paragraph(body,800,305,610,21,30,3,'center','#fff',850,true);const cols=actions.length>3?2:1,w=cols===2?270:420,h=64,startY=actions.length>3?370:470;actions.forEach((a,i)=>{const col=i%cols,row=Math.floor(i/cols),x=cols===2?515+col*300:590;btn(a[0],a[1],x,startY+row*74,w,h,i===0)});}
...
function result(){const b=S.board;cover(IM.setupBg,0,0,W,H,.82);txt('冒險結算',800,58,46,'center','#fff0a5',1000,true);fitTxt(`${b.mapRules?.name||'童話王國'}｜${b.victory==='survival'?'最後生存者':'總資產競賽'}｜${Math.max(1,b.round-1)} 回合`,800,105,720,18,'center','#fff',850,true,13);const ranked=b.players.slice().sort((a,z)=>a.bankrupt!==z.bankrupt?(a.bankrupt?1:-1):netWorth(z)-netWorth(a));ranked.forEach((p,i)=>{const y=150+i*142,alpha=p.bankrupt?.58:.98;stretch(IM['playerSeatP'+(p.id%4)],220,y,540,126,alpha);stretch(IM.roleInfo,770,y,610,126,alpha);contain(IM['portrait'+p.char],238,y+12,104,102,p.bankrupt?.48:1);txt(`${i+1}`,365,y+62,34,'center',i===0?'#ffe071':'#fff',1000,true);fitTxt(`${p.id+1}P ${CHAR_NAMES[p.char]}`,420,y+39,300,23,'left','#fff',1000,true,15);txt(p.bankrupt?'已破產':'完成冒險',420,y+80,16,'left',p.bankrupt?'#ffb7b7':'#dfffdc',850,true);const lands=b.tiles.filter(t=>t.owner===p.id),buildings=lands.reduce((n,t)=>n+t.level,0);fitTxt(`現金 $${p.cash.toLocaleString()}`,810,y+36,230,17,'left','#fff6d2',900,true,12);txt(`土地 ${lands.length}　建築 ${buildings}`,810,y+78,15,'left','#dbeaff',850,true);fitTxt(`總資產 $${netWorth(p).toLocaleString()}`,1135,y+58,330,20,'center',i===0?'#ffe071':'#fff',1000,true,14)});btn('resultHome','返回首頁',420,765,330,78,false);btn('rematch','同設定再戰',850,765,330,78,true)}
let AUDIO_CTX=null,MUSIC_TIMER=0,MUSIC_STEP=0;
function audioGesture(){try{if(!AUDIO_CTX)AUDIO_CTX=new (window.AudioContext||window.webkitAudioContext)();if(AUDIO_CTX.state==='suspended')AUDIO_CTX.resume();if(!MUSIC_TIMER)MUSIC_TIMER=setInterval(musicTick,880)}catch(e){}}
function audioTone(freq,dur,level,type='sine'){if(!AUDIO_CTX||AUDIO_CTX.state!=='running'||level<=0)return;const now=AUDIO_CTX.currentTime,o=AUDIO_CTX.createOscillator(),g=AUDIO_CTX.createGain();o.type=type;o.frequency.setValueAtTime(freq,now);g.gain.setValueAtTime(.0001,now);g.gain.exponentialRampToValueAtTime(Math.max(.0001,level),now+.018);g.gain.exponentialRampToValueAtTime(.0001,now+dur);o.connect(g).connect(AUDIO_CTX.destination);o.start(now);o.stop(now+dur+.03)}
function musicTick(){if(document.hidden||!S.settings||S.settings.master<=0||S.settings.bgm<=0)return;const notes=[523.25,659.25,783.99,659.25,587.33,698.46,880,698.46],gain=.018*(S.settings.master/100)*(S.settings.bgm/100);audioTone(notes[MUSIC_STEP++%notes.length],.7,gain,'triangle')}
function sfx(kind){if(!S.settings||S.settings.master<=0||S.settings.sfx<=0)return;const gain=.055*(S.settings.master/100)*(S.settings.sfx/100),spec={click:[620,.1,'triangle'],dice:[180,.18,'square'],step:[330,.09,'sine'],gain:[880,.28,'triangle'],loss:[145,.35,'sawtooth'],win:[1046,.55,'triangle']}[kind]||[520,.1,'sine'];audioTone(spec[0],spec[1],gain,spec[2])}
function savePrefs(){try{localStorage.setItem(PREF,JSON.stringify(S.settings))}catch(e){}}
function settings(){cover(IM.setupBg,0,0,W,H,.72);txt('聲音與操作',800,105,46,'center','#ffe58a',1000,true);const s=S.settings;btn('master','主音量 '+s.master+'%',500,215,600,68,false);btn('bgm','背景音樂 '+s.bgm+'%',500,305,600,68,false);btn('sfx','遊戲音效 '+s.sfx+'%',500,395,600,68,false);btn('vibrate','骰子震動 '+(s.vibrate?'開':'關'),500,485,600,68,false);txt('音量變更會立即套用並自動保存',800,595,17,'center','#fff6d2',850,true);btn('home','回到首頁',630,650,340,78,true)}
