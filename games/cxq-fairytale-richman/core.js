...
if(q.kind==='tile'){const t=q.tile;title=t.type==='land'?REGION_NAMES[t.region]+'地產':typeName(t.type);if(t.type==='land'){if(t.owner<0){body=`空地售價 $${buyCost(p,t).toLocaleString()}｜基礎租金 $${rentFor({...t,owner:p.id},p).toLocaleString()}｜購買後可升級三階建築`;actions=[['buy','購買土地'],['skip','暫時略過']]}else if(t.owner===p.id){const names=['空地','童話小屋','精緻旅店','豪華地標'];body=t.level<3?`${names[t.level]}｜目前租金 $${rentFor(t,p).toLocaleString()}｜升級費 $${Math.round(t.price*.65).toLocaleString()}`:`${regionOwned(p.id,t.region)?'區域地標已落成':'最高級建築'}｜目前租金 $${rentFor(t,p).toLocaleString()}`;actions=t.level<3?[['upgrade',`升級至 ${names[t.level+1]}`],['skip','完成回合']]:[['skip','完成回合']]}else{const owner=b.players.find(x=>x.id===t.owner);body=`${owner?owner.id+1+'P '+CHAR_NAMES[owner.char]:'對手'}的 Lv${t.level} 地產｜需支付 $${rentFor(t,p).toLocaleString()} 租金`;actions=[['pay',p.shield>0?'使用護盾／結算':'支付租金']]} }else if(t.type==='start'){body='在起點稍作休息，獲得旅費。';actions=[['ok','繼續冒險']]}else{body=`抵達${typeName(t.type)}格`;actions=[['special','查看結果']]}}
