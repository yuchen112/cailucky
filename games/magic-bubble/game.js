(() => {
  ...
  const goal=()=>chapter%3===0?'清空所有泡泡':chapter%3===1?'收集愛心 '+Math.min(collected,15+chapter*2)+'/'+(15+chapter*2):'救出星願 '+rescued+'/5';
  function reset(){
    clock=0;effects=[];tokens=new Set;flight=null;aim=null;pending=null;score=0;collected=0;rescued=0;skill=0;over=false;
    const rows=chapter>=3?6:5,colors=Math.min(6,4+Math.floor(chapter/2));
    grid=Array.from({length:ROWS},(_,r)=>Array.from({length:C},()=>r<rows?Math.floor(Math.random()*colors):-1));
    if(chapter%3===1){const cells=Array.from({length:rows*C},(_,i)=>i).sort(()=>Math.random()-.5);for(const i of cells.slice(0,15+chapter*2))grid[Math.floor(i/C)][i%C]=0;}
    if(chapter%3===2)for(let q=0;q<C;q+=2)tokens.add(key(rows-1,q));
    shots=(mode==='relaxed'?42:32)+chapter;ball=random();next=random();$('#result').hidden=true;sync();
    feedback('第 '+(chapter+1)+' 關 · '+goal());
  }
  function group(r,q){
  ...
  function place(dest){
    ...
    ball=next;if(!available().includes(ball))ball=random();next=random();sync();check();
  }
  function check(){
  ...
  function end(win){over=true;pending={win,at:clock+650};flight=null;aim=null;}
  function result(win){
  ...
  function draw(){
    ...
    for(let r=0;r<ROWS;r++)for(let q=0;q<C;q++)if(grid[r]?.[q]>=0){const p=pos(r,q);bubble(p.x,p.y,grid[r][q]);if(tokens.has(key(r,q))&&rescueArt.naturalWidth)x.drawImage(rescueArt,p.x-19,p.y-23,38,46);}
    if(!flight&&!over){
