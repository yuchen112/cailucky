(() => {
  ...
  function update(dt){
    ...
    for(let i=0;i<towers.length;i++){
      ...
      shots.push({a:P,b:q,kind:t.kind,t:0,duration:.18});t.fired=clock;
      if(d.splash)for(const o of enemies){const z=route(o.p);if(o.hp>0&&Math.hypot(z[0]-q[0],z[1]-q[1])<d.splash)damage(t,o,n);}else damage(t,e,n);
      if(d.slow){e.slow=t.branch==='power'?.3:d.slow;e.slowUntil=clock+1.7;}t.ready=clock+d.rate;
    }
    ...
    enemies=enemies.filter(e=>e.hp>0);shots.forEach(s=>s.t+=dt);shots=shots.filter(s=>s.t<s.duration);effects.forEach(e=>e.t+=dt);effects=effects.filter(e=>e.t<.4);
    if(dead.length)sync();
    if(running&&!spawnLeft&&!enemies.length){running=false;coins+=45;towers.forEach(t=>{if(t?.kind==='trust'&&t.branch==='power')life=Math.min(mode==='relaxed'?30:20,life+t.level);});if(wave>=8){finish(true);return;}CxQ.sound('match');sync();}
  }
  ...
  function draw(){
    ...
    for(const s of shots){const t=s.t/s.duration,px=s.a[0]+(s.b[0]-s.a[0])*t,py=s.a[1]+(s.b[1]-s.a[1])*t-20;sprite(bullets[s.kind],px-15,py-15,30,30);}
